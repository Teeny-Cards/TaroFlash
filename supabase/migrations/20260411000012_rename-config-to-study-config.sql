ALTER TABLE public.decks RENAME COLUMN config TO study_config;
...
CREATE OR REPLACE FUNCTION public.get_member_decks_with_due_count(p_member_id uuid, p_now timestamp with time zone DEFAULT now())
 RETURNS TABLE(id bigint, title text, description text, updated_at timestamp with time zone, has_image boolean, study_config jsonb, cover jsonb, total_cards integer, due_count integer)
 LANGUAGE sql
 STABLE
AS $function$
  select
  ...
  group by d.id, d.title, d.description, d.updated_at, d.has_image, d.study_config, d.cover
