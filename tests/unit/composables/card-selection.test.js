...
describe('useCardSelection', () => {
  ...
  describe('initial state', () => {
    test('starts with empty selection and not in select-all mode', () => {
      const sel = useCardSelection(makeIdsQuery([]))
    ...
    test('total_card_count tracks the ids_query length', () => {
      const sel = useCardSelection(makeIdsQuery([1, 2, 3, 4]))
      expect(sel.total_card_count.value).toBe(4)
    })

    test('total_card_count is 0 when the query has no data', () => {
      const sel = useCardSelection({ data: ref(undefined) })
  ...
  describe('positive selection', () => {
    test('selectCard adds an id once (no duplicates)', () => {
      const sel = useCardSelection(makeIdsQuery([1]))
      sel.selectCard(1)
      sel.selectCard(1)
    ...
    test('deselectCard removes an id', () => {
      const sel = useCardSelection(makeIdsQuery([1, 2]))
      sel.selectCard(1)
      sel.selectCard(2)
      sel.deselectCard(1)
  ...
  describe('select-all mode', () => {
    ...
    test('selected_count = total - deselected in select-all mode', () => {
      const sel = useCardSelection(makeIdsQuery([1, 2, 3]))
      sel.selectAllCards()
